...
function initTrainingView() {
  ...
  // Auto-detect block from today's date and term settings
  const detectedBlock = autoDetectBlock();
  if (detectedBlock) {
    const blockSel = document.getElementById('tr-block');
    if (blockSel) {
      blockSel.value = detectedBlock;
      showBlockInfo();
      toast(`Block ${detectedBlock} auto-selected based on today's date`);
    }
  }
  loadTrainingPlayers();
}
...
function autoDetectBlock() {
  ...
  // Each block is 3 weeks. Calculate which block we are in from term start.
...
window.showBlockInfo = function() {
  ...
  const el  = document.getElementById('tr-block-info');
  if (!el) return;
  if (!val || !BLOCK_INFO[val]) { el.style.display = 'none'; return; }
  const b = BLOCK_INFO[val];
  el.innerHTML = `<strong>${b.title}</strong><br>Focus: ${b.focus}<br>Key behaviours: ${b.behaviours}`;
